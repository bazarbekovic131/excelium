...
(`var/gateway.db`) и правится в /ui → «Подписанты». При переезде на
новую машину переносят `var/` либо «Скачать JSON» → «Заменить из файла»;
пустая база стартует без подписантов. `data/banks.yaml` — справочник
банков для внешнего реестра, читается при каждом старте, остаётся в git.
...
Десять экранов: рабочий стол, очередь заданий, операции с
конструктором, файлы, подписанты, шаблоны Typst, рендер, API-консоль,
настройки, вход. Оформление — плотный рабочий интерфейс с серыми
линиями вместо теней и одним акцентом; шрифт системный, внешних
ресурсов нет.
